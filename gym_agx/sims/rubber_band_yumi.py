...
DIAMETER = 0.0225 * SCALE
DLO_CIRCLE_STEPS = 20
RADIUS = 0.001 * SCALE  # meters
...
GROUND_LENGTH_X = 0.02 * SCALE
GROUND_LENGTH_Y = 0.02 * SCALE
GROUND_HEIGHT = 0.0025 * SCALE
...
POLE_POSITION_OFFSETS = [[0.3 + 0.0*SCALE, 0.01*SCALE], [0.3 + -0.01*SCALE, -0.01*SCALE], [0.3 + 0.01*SCALE, -0.01*SCALE]]
POLE_RADIUS = 0.003 * SCALE

# Ground Parameters
EYE = agx.Vec3(1.5, 0.2, 0.5)
CENTER = agx.Vec3(0.2, 0, 0.2)
UP = agx.Vec3(0., 0., 1.)
MAX_X = 0.01 * SCALE
MAX_Y = 0.01 * SCALE
...
def create_pole(id, sim, position, material):
    ...
    cylinder = create_body(name="cylinder_low_" + str(id), shape=agxCollide.Cylinder(POLE_RADIUS, 0.005*SCALE),
    ...
    rotation_cylinder = agx.OrthoMatrix3x3()
    rotation_cylinder.setRotate(agx.Vec3.Y_AXIS(), agx.Vec3.Z_AXIS())
    cylinder = create_body(name="cylinder_inner_" + str(id), shape=agxCollide.Cylinder(POLE_RADIUS-0.0007*SCALE, 0.005*SCALE),
                           position=agx.Vec3(x,y,0.005* SCALE),
                           rotation=rotation_cylinder,
                           motion_control=agx.RigidBody.KINEMATICS,
                           material=material)
    sim.add(cylinder)
    ...
    rotation_cylinder = agx.OrthoMatrix3x3()
    rotation_cylinder.setRotate(agx.Vec3.Y_AXIS(), agx.Vec3.Z_AXIS())
    cylinder = create_body(name="cylinder_top_" + str(id), shape=agxCollide.Cylinder(POLE_RADIUS, 0.005*SCALE),
                           position=agx.Vec3(x,y,0.01* SCALE),
                           rotation=rotation_cylinder,
                           motion_control=agx.RigidBody.KINEMATICS,
                           material=material)
    sim.add(cylinder)
...
def build_simulation():
    ...
    f0 = agx.Frame()
    f0.setLocalTranslate(0, 0, 0.138)
    f1 = agx.Frame()
    ball_joint = agx.BallJoint(sim.getRigidBody("gripper_r_base"), f0, s1,  f1)
    sim.add(ball_joint)
